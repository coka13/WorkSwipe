...
const resetDB = async () => {
    try {
        ...
        jobOpportunitiesFromJSON.forEach(jobOpp => {
            ...
            if (jobOpp.niceToHave) {
                jobOpp.niceToHave.forEach((tech, index) => {
                    const foundTech = technologiesInDB.find(techDB => techDB.name === tech);
                    jobOpp['niceToHave'][index] = foundTech ? foundTech._id : null;
                });
                jobOpp['niceToHave'] = jobOpp['niceToHave'].filter(id => id !== null);
            }
